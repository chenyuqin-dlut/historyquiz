...
// var that.data.question_num = 5;//活动菜单的最大个数
...
Page({
  ...
  submit: function (e) {
    ...
    wx.request({
      ...
      success: function (res) {
        ...
        var fail = that.data.question_num - right;
        var total = right* that.data.rightfactor - fail*that.data.failfactor;
        var content = "您答对了" + right + "道题，答错了" + fail + "道题,总计收益" + total + "个金币，点击确认返回主菜单";
        wx.showModal({
          title: '提示',
          content: content,
          showCancel: false,
  ...
  touchMove: function (e) {
    ...
    if (touchMove.pageY - touchDot.pageY <= -40 && time < 10) {
      nth++;
      if (tmpFlag && nth < that.data.question_num) { //每次移动中且滑动时不超过最大值 只执行一次
        tmpFlag = false;
        //console.log(nth);
        that.switchQues(nth);
        that.setData({
        ...
        if (nth + 1 >= that.data.question_num) {
          that.setData({
            submit_hidden: false,
          })
        }
      } else if (tmpFlag && nth >= that.data.question_num) {
        wx.showToast({
          title: '已到最后一题了',
          icon: 'success',
          duration: 1000
